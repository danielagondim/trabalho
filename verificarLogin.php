...
session_start();
	...
	require_once("confBD.php");
      ...
      elseif(isset($_COOKIE["loginAutomatico"])){ 	//existe um cookie com nome senha --> login automático
            $log = utf8_encode(htmlspecialchars($_COOKIE["login"]));
 ...
 try{
		...
		if (count($resultados)!=1){	
			...
			setcookie("login", $log); //guarda o login por 90 dias a partir de agora
			if(!empty($lembrar)){
 			    setcookie("loginAutomatico", $senha); //guarda a senha por 90 dias a partir de agora	
			}
		   $_SESSION['auth']=true;
